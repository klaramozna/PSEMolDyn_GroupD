...
int CL::parse_arguments(int argc, char *argsv[], double& end_time, double& delta_t, int& log_level, std::string& input_path, std::string& input_mode){
    desc.add_options()
        ("help,h", "produce help message")
        ("input_mode,m", po::value<std::string>(), R"(Select between modes of input ("cuboid" or "particle"))")
    ...
    if (vm.count("input_mode")) {
        ...
        if (input_string == "cuboid" || input_string == "particles") {
            input_mode = input_string;
        } else {
            Logger::err_logger->error("Input mode is invalid or not specified");
            Logger::console->info("{}", produce_help_message(desc));
            return 1;
        }
    }
