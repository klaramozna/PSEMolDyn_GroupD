 ...
 * @file Particle.h
 *
 * Provides vector operations using simple operators.
 */

class VectorDouble {
private:
    /**
     * Stores values of the vector
     */
...
public:
    ...
    /**
     * Creates a new vector, using the values of the given vector
     * @param vector is the vector, that values are copied from
     */
    explicit VectorDouble(const std::vector<double> &vector);

    /**
     * Creates a vector of size 3, using the values of the given array
    ...
    /**
     * Returns the L2 norm (euclidean norm) of the vector
    ...
    /**
     * Converts the vector to ann array (only available for vectors of size 3)
     * @return
     */
